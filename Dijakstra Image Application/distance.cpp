...
//seed = grid index of destination location (where distance = 0)
{
  ...
  while (queue.getNext(p)) {
	  int k=p/Z, j=(p-k*Z)/Y, i=p-k*Z-j*Y;                      //Convert 1D index to 3D indeces
	...
	if (k==0 && !visited[q=p-Z+gridsize]) {         //Visit -Z neighbor
      visited[q]=1;
      if (VALID(i,j,(zsize-1))) {dist[q]=dist[p]+1; queue.append(q);}
    }

		
	  

    if (k==zsize-1 && !visited[q=p+Z-gridsize]) {   //Visit +Z neighbor
      visited[q]=1;
